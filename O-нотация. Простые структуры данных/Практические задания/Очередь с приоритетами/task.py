...
class PriorityQueue:
    ...
    def __init__(self):
        ...
        self.priority_queue = {value: deque() for value in range(self.HIGH_PRIORITY, self.LOW_PRIORITY + 1)}
    ...
    def enqueue(self, elem: Any, priority: int = 0) -> None:
        ...
        if priority not in range(self.HIGH_PRIORITY, self.LOW_PRIORITY + 1):
            raise ValueError("Такого приоритета нет")
    ...
    def peek(self, ind: int = 0, priority: int = 0) -> Any:
        ...
        if not isinstance(ind, int):
            raise TypeError("Индекс не целочисленный")
    ...
    def __len__(self):
        ...
        s = 0
        for value in self.priority_queue.values():
            s += len(value)
        return s
